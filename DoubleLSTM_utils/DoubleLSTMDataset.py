...
class DoubleLSTMDataset(Dataset):
    def __init__(self, search_terms, product_descriptions, relevances, word2vec_search, word2vec_desc):
        """
        Constructor for DoubleLSTMDataset class
        ...
        :param word2vec_search: word2vec model for search terms
        :param word2vec_desc: word2vec model for descriptions
        """
        self.search_terms = search_terms
        self.product_descriptions = product_descriptions
        self.relevances = relevances
        self.word2vec_search = word2vec_search
        self.word2vec_desc = word2vec_desc
    ...
    def __getitem__(self, idx):
        ...
        # convert to vectors using appropriate word2vec model
        search_term_vectors = [self.word2vec_search.wv[token] for token in search_term if
                               token in self.word2vec_search.wv]
        product_description_vectors = [self.word2vec_desc.wv[token] for token in product_description if
                                       token in self.word2vec_desc.wv]
